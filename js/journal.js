...
function openJournal() {
  ...
  gsap.to('#portal-flash', {
    ...
    onComplete: () => {
      ...
      if (window._centerPanCanvas) window._centerPanCanvas();
...
// PANNING CANVAS
// ──────────────────────────────────────────────
function setupPanCanvas() {
  const bookPaper   = document.getElementById('book-paper');
  const paperCanvas = document.getElementById('paper-canvas');
  const pan  = { x: 0, y: 0 };
  const drag = { active: false, startX: 0, startY: 0 };

  function centerCanvas() {
    pan.x = (window.innerWidth  - 5000) / 2;
    pan.y = (window.innerHeight - 5000) / 2;
    paperCanvas.style.transform = `translate(${pan.x}px,${pan.y}px)`;
  }
  window._centerPanCanvas = centerCanvas;

  // Mouse
  bookPaper.addEventListener('mousedown', e => {
    if (e.target.closest('#bm-close')) return;
    drag.active = true;
    drag.startX = e.clientX - pan.x;
    drag.startY = e.clientY - pan.y;
    bookPaper.classList.add('dragging');
  });
  ...
  window.addEventListener('mouseup', () => {
    drag.active = false;
    bookPaper.classList.remove('dragging');
  });

  // Touch
  bookPaper.addEventListener('touchstart', e => {
    const t = e.touches[0];
    drag.active = true;
    drag.startX = t.clientX - pan.x;
    drag.startY = t.clientY - pan.y;
  }, { passive: true });
  bookPaper.addEventListener('touchmove', e => {
    if (!drag.active) return;
    const t = e.touches[0];
    pan.x = t.clientX - drag.startX;
    pan.y = t.clientY - drag.startY;
    paperCanvas.style.transform = `translate(${pan.x}px,${pan.y}px)`;
  }, { passive: true });
  bookPaper.addEventListener('touchend', () => { drag.active = false; });
}
